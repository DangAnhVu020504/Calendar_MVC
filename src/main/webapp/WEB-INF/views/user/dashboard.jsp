        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventClick: function(info) {
                    ...
                    if (event.start) {
                        selectedDateStr = event.start.toISOString().split('T')[0];
                        localStorage.setItem('selectedDate', selectedDateStr);
                        openTodoPanel();
                        document.getElementById('selectedDate').textContent = new Date(selectedDateStr).toLocaleDateString('vi-VN');
                        document.getElementById('addEventLink').href = '${pageContext.request.contextPath}/schedule/new?date=' + selectedDateStr;
            ...
            // Yêu cầu quyền Notification ngay khi tải trang
            if (Notification.permission !== "granted") {
                Notification.requestPermission();
            }

            // Thông báo sự kiện dựa vào thời gian kết thúc
            calendar.getEvents().forEach(function(event) {
                ...
                // Thông báo chỉ cho sự kiện chưa kết thúc quá 5 phút
                ...
                notifyTimes.forEach(function(nt) {
                    ...
                    function showTimedNotification(title, body) {
                        if (Notification.permission === "granted") {
            ...
            // Kiểm tra và thông báo mỗi phút cho sự kiện sắp diễn ra trong 5 phút
            setInterval(function() {
                ...
                calendar.getEvents().forEach(function(event) {
                    ...
                    if (diff >= 0 && diff <= 300000) {  // 5 phút tới
                        if (Notification.permission === "granted") {
                            var n = new Notification(
                                "Nhắc nhở sự kiện: " + (event.title || "Không có tiêu đề"),
                                { body: "Sự kiện sắp diễn ra lúc " + start.toLocaleString('vi-VN') }
                            );
                            setTimeout(() => n.close(), 5000);
                        }
                    }
