...
.calendar.sidebar-active {
    margin-right: 320px; /* nhường chỗ sidebar */
    transform: none; /* bỏ scale */
    height: 100%; /* giữ chiều cao */
}
   ...
   <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                customButtons: {
                    today: {
                        ...
                        click: function() {
                            ...
                            document.getElementById('addEventLink').href = contextPath + '/admin/new-schedule?date=' + selectedDateStr;
                ...
                events: [
                    <c:forEach var="schedule" items="${schedules}" varStatus="status">
                        {id: '${schedule.id}', title: '${schedule.title}', start: '<fmt:formatDate value="${schedule.startDateAsDate}" pattern="yyyy-MM-dd\'T\'HH:mm:ss"/>', end: <c:choose><c:when test="${schedule.endDateAsDate != null}">'<fmt:formatDate value="${schedule.endDateAsDate}" pattern="yyyy-MM-dd'T'HH:mm:ss"/>'</c:when><c:otherwise>null</c:otherwise></c:choose>, description: '${schedule.description}', type: '<c:choose><c:when test="${schedule.type == 'LONG_TERM'}">Dài hạn</c:when><c:when test="${schedule.type == 'SHORT_TERM'}">Ngắn hạn</c:when><c:when test="${schedule.type == 'RECURRING'}">Lặp lại</c:when></c:choose>', className: 'priority-${schedule.priority.name().toLowerCase().replace("_", "-")}' }<c:if test="${!status.last}">,</c:if>
                    </c:forEach>
                ],
                ...
                eventClick: function(info) {
                    ...
                    document.getElementById('editEvent').href = contextPath + '/admin/edit-schedule/' + event.id + '?source=dashboard';
            ...
            calendarEvents.forEach(function(event) {
                ...
                // Chỉ bỏ qua sự kiện đã kết thúc hơn 5 phút
                if ((end.getTime() - now.getTime()) < -300000) return;

                var notifyTimes = [
                    {diff: 24*60*60*1000, label: "Còn 1 ngày đến hạn kết thúc"},
                    {diff: 5*60*60*1000, label: "Còn 5 giờ đến hạn kết thúc"},
                    {diff: 1*60*60*1000, label: "Còn 1 giờ đến hạn kết thúc"},
                    {diff: 0, label: "Sự kiện đã kết thúc!"}
                ];

                notifyTimes.forEach(function(nt) {
                    var notifyAt = new Date(end.getTime() - nt.diff);
                    var timeout = notifyAt.getTime() - now.getTime();

                    if (timeout > 0) {
                        setTimeout(function() {
                            var title = event.title || "Không có tiêu đề";
                            var body = "";

                            if (nt.diff === 0) {
                                body = (event.extendedProps.description || "") + "\nKết thúc lúc: " + end.toLocaleString('vi-VN');
                            } else {
                                body = nt.label + " (" + end.toLocaleString('vi-VN') + ")";
                            }

                            // Thêm tag duy nhất dựa trên event id và loại thông báo
                            var tag = 'event-' + event.id + '-notify-' + nt.diff;
            ...
            setInterval(function() {
                ...
                calendarEvents.forEach(function(event) {
                    ...
                    // Kiểm tra nếu sự kiện sắp diễn ra trong vòng 5 phút tới
                    var diff = start.getTime() - now.getTime();
                    if (diff >= 0 && diff <= 300000) {  // 5 phút = 300000 ms
                        showNotification(
                            "Nhắc nhở sự kiện: " + (event.title || "Không có tiêu đề"),
                            "Sự kiện sắp diễn ra vào lúc " + start.toLocaleString('vi-VN')
                        );
                    }
                });
            ...
            function updateTodoList(dateStr, calendar) {
                ...
                events.forEach(function(event, index) {
                    ...
                    editLink.href = contextPath + '/admin/edit-schedule/' + event.id + '?source=dashboard';
