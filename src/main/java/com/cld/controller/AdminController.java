...
public class AdminController {
    ...
    public String dashboard(Model model, HttpSession session) {
        ...
        List<Schedule> adminSchedules = scheduleService.getUserSchedules(currentUser.getId());
        ...
        model.addAttribute("schedules", adminSchedules);
    ...
    public String editScheduleForm(@PathVariable Long id, @RequestParam(value = "source", required = false) String source, Model model, HttpSession session) {
        User currentUser = (User) session.getAttribute("user");
        if (currentUser == null || currentUser.getId() != 1) {
            return "redirect:/login";
        }
    ...
    public String createScheduleForAdmin(
            @RequestParam("title") String title,
            @RequestParam("description") String description,
            @RequestParam("startDate") String startDate,
            @RequestParam(value = "endDate", required = false) String endDate,
        ...
        try {
            ...
            if (schedule.getType() == ScheduleType.RECURRING) {
                ...
                schedule.setRecurring(true);
                schedule.setRecurringPattern(recurringDays != null ? String.valueOf(recurringDays) : null);
            } else {
                schedule.setEndDate(endDate != null && !endDate.isEmpty() ? LocalDateTime.parse(endDate, formatter) : null);
                schedule.setRecurring(false);
                schedule.setRecurringPattern(null);
            }
            ...
            model.addAttribute("priorities", Priority.values());
            model.addAttribute("scheduleTypes", ScheduleType.values());
            return "admin/new-schedule"; 
        }
    }

    @GetMapping("/new-schedule")
    public String newScheduleForm(@RequestParam(value = "date", required = false) String date,
